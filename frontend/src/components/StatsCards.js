...
const StatsCards = ({ income, expenses, balance }) => {
  const cards = [
    ...
    },
    {
      ...
      color: balance >= 0 ? 'from-blue-500 to-purple-600' : 'from-orange-500 to-red-600',
      bgColor: balance >= 0 ? 'from-blue-50 to-purple-50' : 'from-orange-50 to-red-50',
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((card, index) => (
        <div
          key={index}
          className={`bg-gradient-to-br ${card.bgColor} backdrop-blur-xl rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}
        >
